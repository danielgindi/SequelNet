...
namespace dg.Sql
{
    public class Query
    {
        ...
        {
            ConnectorBase connection = ConnectorBase.NewInstance();
            try
            {
                if (_QueryMode == QueryMode.ExecuteStoredProcedure)
                {
                    return connection.ExecuteReader(BuildDbCommand(connection), true);
                }
                else
                {
                    return connection.ExecuteReader(BuildCommand(connection), true);
                }
            }
            catch (System.Exception ex)
            {
        ...
        public object ExecuteScalar()
        {
            using (ConnectorBase connection = ConnectorBase.NewInstance())
            {
                if (_QueryMode == QueryMode.ExecuteStoredProcedure)
                {
                    return connection.ExecuteScalar(BuildDbCommand(connection));
                }
                else
                {
                    return connection.ExecuteScalar(BuildCommand(connection));
                }
            }
        }
        public object ExecuteScalar(ConnectorBase connection)
        {
            if (connection == null) return ExecuteScalar();
            else
            {
