 ...
 *  @AfterUpdateQuery:      Code to execute after generating the Update query.
 ...
 *  @NullableEnabled		Enable nullable properties (#nullable enable)
 *  @ComponentModel			Add component model annotations
 */
 ```
...
##### Keywords that affect the AbstractRecord<> structure: (Case Insensitive)
 ...
 `QueryBoundary` - marks a column that scopes queries: it must occur in the WHERE clause of SELECT, UPDATE, and DELETE queries, and in the INSERT values of INSERT and UPSERT queries; violations are handled by `Query.QueryBoundaryViolationHandler`
